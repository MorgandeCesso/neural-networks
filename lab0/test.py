...
np.set_printoptions(threshold=np.inf)  # шоб np array полностью выводился


class Neuron:
    def __init__(
        self, input_size: int, learning_rate: float = 0.5
    ) -> None:  # инит-функция с параметром для весов и альфой
        self.weights: np.ndarray = np.random.rand(
            input_size
        )  # Весовые коэффициенты (сделать через uniform)
    ...
    def predict(self, input_data: np.ndarray) -> int:
        weighted_sum: float = np.dot(
            input_data, self.weights
    ...
    def train(self, input_data: np.ndarray, target: np.ndarray) -> None:
        for i in range(len(input_data)):
            weighted_sum: float = np.dot(
                input_data[i], self.weights
            )  # Взвешенная сумма для текущего образца
            activated_output: int = self.threshold_activation(
                weighted_sum
            )  # Выходные данные активируются
            error: int = target[i] - activated_output  # Считаем ошибку
            if error != 0:
                self.weights += (
                    input_data[i] * error * self.learning_rate
                )  # Корректируем веса, если ошибка не = 0

    def train_until_perfect(
        self, images: np.ndarray, labels: np.ndarray, max_epochs: int = 100
    ) -> None:
        for epoch in range(max_epochs):  # Во избежание зацикливания лимит эпох 100
            ...
            wrong_count: int = sum(
                self.predict(images_shuffled[i]) != labels_shuffled[i]
    ...
    for filename in os.listdir(folder):
        ...
        try:
            with Image.open(img_path) as img:
                img: Image.Image = img.convert(
                    "L"
                )  # Конвертация в черно-белое изображение
                img: Image.Image = img.resize(
                    (100, 100)
                )  # Изменение размера изображения до 100x100
                images.append(
                    np.asarray(img).flatten() / 255.0
                )  # Преобразование изображения в нормализованный одномерный массив
                label: int = (
                    0 if int(filename.split(".")[0]) < 20 else 1
                )  # Присвоение метки в зависимости от имени файла
                labels.append(label)
...
class NeuronCanvas(tk.Tk):
    def __init__(self, model: Neuron) -> None:
        super().__init__()
    ...
    def setup_buttons(self) -> None:
        test_button: tk.Button = tk.Button(
            self, text="Проверить", command=self.test_image
        )
        test_button.pack(side=tk.BOTTOM)
        clear_button: tk.Button = tk.Button(
            self, text="Очистить", command=self.clear_canvas
        )
...
if __name__ == "__main__":
    ...
    model: Neuron = Neuron(10000)  # 100x100 пикселей
    model.train_until_perfect(images, labels)
    app: NeuronCanvas = NeuronCanvas(model)
    app.title("Нолик или Крестик?")
    app.mainloop()
