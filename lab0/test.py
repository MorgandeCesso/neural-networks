...
class PerceptronModel:
    ...
    def predict(self, input_data: np.ndarray) -> int:
        weighted_sum: float = np.dot(input_data, self.weights)
        return self.threshold_activation(weighted_sum)

    def train(self, input_data: np.ndarray, target: np.ndarray) -> None:
        for i in range(len(input_data)):
            weighted_sum: float = np.dot(input_data[i], self.weights)
            activated_output: int = self.threshold_activation(weighted_sum)
            error: int = target[i] - activated_output
            if error != 0:
                self.weights += input_data[i] * error * self.learning_rate
    ...
    def train_until_perfect(self, images: np.ndarray, labels: np.ndarray, max_epochs: int = 100) -> None:
        for epoch in range(max_epochs):
            ...
            wrong_count: int = sum(self.predict(images_shuffled[i]) != labels_shuffled[i] for i in range(len(images_shuffled)))
            epoch_info: str = f"Epoch {epoch+1}, wrong predictions: {wrong_count}"
            if wrong_count == 0:
                print(epoch_info)
                print(f"Perfectly trained after {epoch+1} epochs")
                return
            self.train(images_shuffled, labels_shuffled)
            print(epoch_info)
    ...
    for filename in os.listdir(folder):
        ...
        try:
            with Image.open(img_path) as img:
                img: Image.Image = img.convert("L")
                img: Image.Image = img.resize((100, 100))
                images.append(np.asarray(img).flatten() / 255.0)
                label: int = 0 if int(filename.split('.')[0]) < 20 else 1
                labels.append(label)
