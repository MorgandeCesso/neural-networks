...
class NumberProcessor():
    def __init__(self, folder, initial_learning_rate=0.01, hidden_size=20, num_classes=10, input_size=10000, regularization_lambda=0.01, decay_rate=0.999):
    ...
    def guess(self, image):
        ...
        _, _, _, A2 = self.forward(image)  # Прямое распространение
    ...
    def train(self, images, labels):
        ...
        while epoch < max_epochs:
            ...
            Z1, A1, Z2, A2 = self.forward(images)  # Прямое распространение
            loss = self.cross_entropy_loss(Y, A2)  # Вычисление потерь
            dW1, db1, dW2, db2 = self.backward(images, Y, Z1, A1, Z2, A2)  # Обратное распространение
...
if __name__ == "__main__":
    folder_path = "dataset"
    processor = NumberProcessor(folder_path, initial_learning_rate=0.01)  # Начальная скорость обучения
